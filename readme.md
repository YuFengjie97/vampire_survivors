...
- 主脚本,enemy.gd(是的,子敌人脚本都相同,并且过于简单,以至于子敌人.tscn可以直接使用enemy.gd.这里主要是练习如何封装)
- 主场景,enemy.tscn
...
在world-enemySpawn中添加敌人波

## lesson 15
> 死亡菜单,开始场景,bgm

bgm
- process mode-always永远播放
- player 死亡时,信号通知停止
...
失败与成功
- 没有按照教程里通过判断时间来确定
- hurt_box中area_enter时获取进入的area,即hit_box,通过owner获取enemy,通过enemy中enemy_type判断是普通敌人还是死神.
被死神杀死才是成功,普通敌人杀死为失败